...
# Google spreadsheet de destino  "Listado fotos Agustí Umbert 2025" 
puntuaciones_sheet_id = '1lplThhHsQlSc6qsBA_kHMS8mE_WxFVN3gy5Qy_hz_cY'
...
# Google spreadsheet con  Listado de participantes "Autores Premio Agusti Umbert 2025" 
personas_sheet_id = '1mfg9LNb9lBBQLxPnR80z1Qli6jWK0h62ko-1LBi0ixY'
...
def copyphotos_and_number(api, source_dir, dest_dir):
    ...
    # Sort files alphabetically by name as uppercase
    files.sort(key=lambda x: x['name'].upper())
    ...
    for position, file in enumerate(files, start=1):
        ...
        #Note: original_filename is Unicode not UTF-8 (see below)
        ...
        # Convert to UTF-8 Hidrive filenames are Unicode and then encoded URLs 
        UTF8_original_filename= unicodedata.normalize('NFC', original_filename)
        file_correspondences[UTF8_original_filename] = new_filename
...
def format_month_sheet(sheet, num_rows):
    ...
    column_headers = [["NUM.", "AUTOR", "NOMBRE ARCHIVO FOTO", "SOCIO", "FEDERADO" ]]
...
def match_author_names(file_correspondences, puntuaciones_sheet):
    ...
    for origen_filename, number_filename in file_correspondences.items():
        ...
        names_FromFile_List= processFilename.SplitAndEdit(photo_file_name)

        max_similarity = 0.0
        max_similarity_index = -1
        for photo_file_name_part in names_FromFile_List:
            # Initialize variables to track maximum similarity ratio and corresponding index
            matcher.set_seq2(photo_file_name_part)

            # Compare with each author name and compute Levenshtein similarity ratio
            for j, author_row in enumerate(personas[1:]):
                author = author_row[0]
                matcher.set_seq1(author)
                similarity = matcher.ratio()
                if similarity > max_similarity:
                    max_similarity = similarity
                    max_similarity_index = j + 1
        ...
        if max_similarity > MINIMUM_SIMILARITY:
            nearest_author_name = personas[max_similarity_index][0]
            socio = personas[max_similarity_index][3]
            federado = personas[max_similarity_index][4]
        else:
            nearest_author_name = "**NO IDENTIFICADO**"
            socio = "?"
            federado = "?"

        matched_names.append([photo_number, nearest_author_name, origen_filename, whenSI(socio, 'socio'), whenSI(federado,'federado')])

    return matched_names

def whenSI(value, text):
    if value == 'SI' or value == 'SÍ':
        return text
    else: 
        return ''
...
def get_command_line_arguments():
    if len(sys.argv) >= 2:
        contest_year = sys.argv[1]
    else:
        # If not enough parameters are provided, ask the user
        if len(sys.argv) < 2:
            contest_year = input("Entre el año del concurso (Ex. 2025 ): ")
        else:
            contest_year = sys.argv[1]
    folder_name = "Concurso " + contest_year

    return contest_year, folder_name
...
load_directory ="/users/photosagrera/PREMI AGUSTI UMBERT/"
destination_directory = "/users/photosagrera/PREMI AGUSTI UMBERT/Concurso 2025/Numeradas"


def main():
    sheet_tab_name, folder_name = get_command_line_arguments()
    source_directory = load_directory + folder_name + "/Originales"
    print(f"Procesando la carpeta: {folder_name}")
    ...
    puntuaciones_worksheet=create_month_sheet(sheet_tab_name, number_of_authors+1)
    ...
    puntuaciones_worksheet.update(matched_names, f"A2:E{len(matched_names)+1}")
    print(f"Consultar listado en la pestaña: {sheet_tab_name} del Google 'Spreadsheet Listado fotos Agustí Umbert 2025'")
    ...
    print("**Proceso completedo con éxito!")
